...
class ProductsApp(tk.Tk):
            ...
            if column_index == 0:  # Barcode validation
                ...
                if new_value in all_barcodes:
                    messagebox.showerror(
                        "Error", "Barcode already exists.", parent=self
                    )
                    entry.destroy()
                    return
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Error", "Price must be a number.", parent=self
                    )
                    entry.destroy()
                    return
                ...
                try:
                    ...
                    messagebox.showerror(
                        "Error", "Inventory must be an integer.", parent=self
                    )
                    entry.destroy()
                    return
    ...
    def save_to_csv(self):
        ...
        try:
            ...
            messagebox.showinfo(
                "Success", "Changes saved successfully to products.csv."
            )
    ...
    def load_products(self):
        ...
        if not os.path.exists(filepath):
            messagebox.showwarning(
                "File Not Found",
                f"File '{filepath}' not found. A new one will be created.",
            )
        ...
        try:
            with open(filepath, mode="r", encoding="utf-8") as infile:
                ...
                header = next(reader)
                if header != ["barcode", "name", "price", "inventory"]:
                    messagebox.showerror(
                        "Format Error",
                        "CSV file has incorrect header.",
                    )
                    return
                for i, row in enumerate(reader):
                    if len(row) == 4:
                        ...
                        messagebox.showwarning(
                            "Invalid Row",
                            f"Row {i + 2} in '{filepath}' is malformed and will be ignored.",
                        )
    ...
    def add_product(self):
        ...
        if not all([barcode, name, price]):
            messagebox.showerror(
                "Error", "Barcode, name, and price fields are required."
            )
            return
        ...
        try:
            ...
            messagebox.showerror(
                "Error", "Price and inventory must be numbers."
            )
            return
    ...
    def write_products_to_csv(self, products):
        ...
        if not os.path.exists(filepath):
            with open(filepath, "w", newline="", encoding="utf-8") as f:
                 writer = csv.writer(f)
                 writer.writerow(["barcode", "name", "price", "inventory"])
